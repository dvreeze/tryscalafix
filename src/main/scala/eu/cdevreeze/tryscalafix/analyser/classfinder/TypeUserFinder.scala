...
/**
 * Finder of definitions of non-abstract types that use any of the types given as search input symbols.
 *
 * @author
 *   Chris de Vreeze
 */
final class TypeUserFinder(val searchInputSymbols: Seq[Symbol], val classCategoryDisplayName: String)
    extends ClassFinder {
  ...
  override def findClasses(implicit doc: SemanticDocument): ClassFinder.SearchResult = {
    ...
    val matchingTerms: Seq[Term] = doc.tree.filterDescendants[Term] { t =>
      ...
      symbolMatcher.matches(t.symbol.owner) && t.findFirstAncestor[Defn.Def]((_: Defn.Def) => true).nonEmpty
    }
